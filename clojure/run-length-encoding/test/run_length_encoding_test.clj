...
(deftest encode-string-pair
  (testing "encode string with a pair of the same character"
    (is (= (rle/run-length-encode "AA") "2A"))))
...
(deftest decode-string-pair ()
  (testing "decode string with a pair of the same character"
    (is (= (rle/run-length-decode "2A") "AA"))))
